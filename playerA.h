...
public:
	...
	void OnDraw() {
		...
		DrawImage(pos.x - camera.GetPos().x - gun_rotated.getwidth()/2, pos.y - camera.GetPos().y - gun_rotated.getheight()/2 + 20, &gun_rotated,1,0,BLACK);
	...
	void PlatformTopCollide(vector<Platform> platform_list) {
		if (speed.y >= 0) {//碰撞检测
